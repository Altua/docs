...
This page covers the Windows-specific installation details for the current .NET 9 release.

<Info>
Grunt is self-contained and no longer requires the VSTO Runtime or .NET Framework on the target machine. If you are upgrading from the .NET Framework 4.7.2 version, see [Upgrading from .NET Framework](/powerpoint-add-in/it/upgrading-from-dotnet-framework).
</Info>
