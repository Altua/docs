...
This is the recommended mode for users installing Grunt themselves. For Intune deployments, use per-machine installation in most cases. Choose per-user when you want Intune to handle only the initial deployment and Grunt to handle automatic updates.
...
For fully managed deployments through Intune, SCCM, or similar systems, use per-machine installation and distribute all updates through your deployment tooling.

If you use Intune for the initial deployment and want Grunt to handle updates, use per-user installation in the user's Windows context and leave automatic updates enabled. See [Deployment and installation](/powerpoint-add-in/it/deployment-and-installation) for help choosing an installation model.
