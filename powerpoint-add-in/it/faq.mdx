...
<AccordionGroup>
  ...
  <Accordion title="Can we roll back to a previous version if a Grunt update causes issues?">
    ...
    Large organizations should be on the Stable or Managed release channel so new functionality has been validated broadly before reaching their users. See [update management](/powerpoint-add-in/it/update-management) for details.
  </Accordion>
