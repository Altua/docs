...
For Intune deployments, we recommend per-machine installation in most cases. Choose the model based on how you want to manage updates:

| Update ownership | Recommended installation model |
| --- | --- |
| You manage the initial deployment and all updates through Intune. | Per-machine. Grunt auto-updates are disabled, so you deploy each update through Intune. |
| You use Intune for the initial deployment and want Grunt to handle updates automatically. | Per-user. Run the installation in the user's Windows context and leave automatic updates enabled. |

For users installing Grunt themselves, the default per-user install is the standard choice. It does not usually require admin rights and enables auto-updates by default.
...
You can deploy Grunt with common software distribution platforms such as Intune, SCCM, or similar tooling. Use the per-machine model for a fully managed rollout where you distribute both the initial installation and subsequent updates.

If you want Grunt to handle updates after the initial deployment, deploy the default per-user installer through Intune or another MDM tool in the user's Windows context. This keeps Grunt under `%LOCALAPPDATA%` and writes settings to `HKEY_CURRENT_USER`. Leave automatic updates enabled so Grunt can install subsequent updates.
